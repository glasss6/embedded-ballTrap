 ...
 * @brief Source code for accelerometer module
...
#define GSCALE          2       /// Default scale is 2G
#define MMA8452_ADDRESS 0x1D    //  0x1D if SA0 is high, 0x1C if low
#define WHO_AM_I        0x0D    //  Register holds value if device is online
#define DEVICE_FOUND    0x2A    //  Value that says device is online
#define CTRL_REG1       0x2A    //  Control register 1
#define XYZ_DATA_CFG    0x0E    //  XYZ Data configuration register
#define OUT_X_MSB       0x01

#define I2C_CLOCK_SPEED 100000  //  Default I2C clock speed

struct accelerometer_data {
    int16_t raw_XYZ[3]; ///< holds the raw XYZ data from the sensor
    float g_XYZ[3];     ///< holds the converted XYZ data
    void(*callback)(float x, float y, float z); // callback fnction pointer
};
...
int8_t Accelerometer_Init(uint16_t period, void(*callback)(float x, float y, float z)) {
    ...
    if(I2C_MAX_TX_SIZE < 2) { /// Return error if TX size is not at least 2
        return ERROR_TX_SIZE; // transmit buffer not long enough
    }
    else if(I2C_MAX_RX_SIZE < 6) { /// Return error if RX size is not at least 6
        return ERROR_RX_SIZE; // receive buffer not long enough
    }
